...
def decode_seq(seq, type, args):
    ...
    # else:
    #     raise ValueError
    seq = seq.reshape(-1, (args.word_len + 2))
    ...
    for text_ins_seq in seq:
        ...
        for index in text_ins_seq[2:]:
            # if index == args.recog_pad_index or index == args.num_char_classes:
            #     continue
            # if index == args.padding_index:
            #     break
            ...
            if index == args.recog_pad_index - 1:
                # 遇到unknown，忽略
                continue
            recog.append(args.letters[index - args.num_bins])
